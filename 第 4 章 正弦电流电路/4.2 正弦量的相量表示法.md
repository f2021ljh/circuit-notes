...
- 微分规则：设 $f(t)$ 的向量为 $\dot{A}_{\mathrm{m}}$，即 $f(t)=\operatorname{Re}[\dot{A}_{\mathrm{m}}\mathrm{e}^{\mathrm{j}\omega t}]$，则 $\dfrac{\mathrm{d}}{\mathrm{d}t}f(t)=\operatorname{Re}[\mathrm{j}\omega \dot{A}_{\mathrm{m}}\mathrm{e}^{\mathrm{j}\omega t}]$，即 $\dfrac{\mathrm{d}}{\mathrm{d}t}f(t)$ 的相量为 $\mathrm{j}\omega \dot{A}_{\mathrm{m}}$。因此，正弦量对时间求导运算，可变换为复数的乘法运算。